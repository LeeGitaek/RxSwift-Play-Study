...
//  RxswiftPlay
//
//  Created by gitaeklee on 2021/01/17.
//
...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var rxac: UIButton!
    ...
    private func updateUI(photos: [UIImage]) {
            savebtn.isEnabled = photos.count > 0 && photos.count % 2 == 0
            clearbtn.isEnabled = photos.count > 0
            savebtn.isEnabled = photos.count < 6
            title = photos.count > 0 ? "\(photos.count) 개 의 사진" : "프로필 사진 대기중"
    }
    
    @IBAction func savebtn(_ sender: Any) {
        
        images.value.append(UIImage(named: "profile.png")!)
